...
| `--shard <k>/<n>` | _unset_ | Only run tickets whose slug hashes to shard *k* of *n* (0-based). Lets *n* runners on separate clones split the same stages without picking the same ticket; a ticket stays in its shard as it moves through stages. |
| `--only <slugs>` | _unset_ | Comma-separated ticket slugs (no sequence prefix, no `.md`). Restricts selection to exactly these tickets, overriding the runner's priority/sequence ranking within `--stages` entirely — for hand-assigning specific tickets to a runner, e.g. splitting work between two clones without relying on shard hashing. A named ticket still must sit in one of the requested `--stages`. Composes with `--shard` (a ticket must pass both). |
...
Triage never routes a reproducible failure to `backlog/` and never resolves one with `it.skip`, a deleted test, or a loosened assertion. To keep the same failure from being re-triaged from cold by every subsequent ticket, triage records it in `tickets/.pre-existing-known.md`. Each entry is one line: a backticked test signature, then free prose, then a tail that must be the last thing on the line — ``- `<signature>` — <prose> → <slug> | <state> | <YYYY-MM-DD>`` (the arrow may be `→` or `->`). `<state>` is `in-flight` (a `fix/` ticket is live), `blocked` (a `blocked/` ticket is), or `one-off` (investigated and resolved with no ticket filed — a deliberate permanent record, whose slug position reads `no tracking ticket`). Before dispatching, the runner consults the ledger and short-circuits when an `in-flight`/`blocked` entry's signature appears in the report; `one-off` entries suppress nothing. The prose carries its weight: a signature is usually a file path, one file can hold two unrelated failures, and the match is on the signature alone — so the entry should say which tests, which error text, and how to confirm it is this failure rather than a new one at the same path. A suppressed report is written to `tickets/.logs/pre-existing-error.suppressed.<ts>.log` before it is deleted, so a wrong match is recoverable.
...
**Ledger reconciliation.** The ledger is committed (not gitignored) so it persists across runs and agents — which means it also needs sweeping, or it accumulates entries for failures long since fixed and starts suppressing re-triage of *genuine regressions* that reuse the same test path. Triage removes an entry only when it personally re-runs the test and finds it gone, or lands a fix in place; the common case — a failure that flows `fix → implement → review → complete` through the normal pipeline — leaves its entry orphaned, since nothing re-reads the ledger when the tracking ticket lands. So at the start of every run (alongside the completed-ticket sweep, and gated by the same `--no-prune-completed` / `--dry-run` flags), the runner reconciles the ledger against live ticket state and commits the change as `tess: prune <n> resolved known-failure ledger entr(y/ies)`. An entry is dropped only when both halves hold: its state is `in-flight` or `blocked`, **and** its slug resolves on the board to `complete/` — including via `.pruned-tickets.jsonl`, so a completed-then-swept tracker still prunes. Everything else is kept: entries still live in `fix`/`plan`/`implement`/`review`/`blocked`/`backlog`, `one-off` records, and — reported on the console rather than acted on — entries whose slug resolves nowhere. Absence is not evidence a fix landed; it is equally a typo, a rename, or a tracker on another repository's board, and none of those should delete accumulated triage detail. Pruning is safe-conservative — it removes only the *suppression*, never re-detection: if a failure is in fact still broken after its tracker completed, the next ticket that trips it reproduces it at HEAD, re-files, and re-adds the entry. A ledger with entry-shaped lines that parse to nothing is reported too: that state reads as "empty" everywhere, and went unnoticed for a year of entries once already.
