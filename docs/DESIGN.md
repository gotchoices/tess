...
1. **Backfill** — verify every backlog ticket carries `severity:` / `likelihood:` / `tradeoffs:` headers and at least one specification anchor (see agent rules); derive missing ones from the ticket body and code, never inventing a code the specification doesn't have.
...
3. **Rank** — order the surviving queue by release first (the top level of `backlog/` is current; release folders follow in `releases.md` order), then severity × likelihood, then cost, surfacing the handful of genuine decisions.
4. **Propose declines** — list the top-level tickets that have waited past a threshold (default 60 days) for the human to decline, promote or defer. A ticket nobody has chosen to act on for that long is evidence it sits below the interest threshold, and asking about it periodically keeps the queue from only growing. Age counts from the ticket's arrival at the top level, not its filing: re-sequencing is the gardener's own bookkeeping and must not reset it, while a ship or a parking-back from another stage is a fresh choice to keep the ticket current and does.
5. **Execute human feedback** — when the human declines tickets (via feedback the gardener is invoked with, or by prior conversation), the gardener deletes them **and plants an accepted-tradeoff `NOTE:` comment at the code site** (see agent rules, *Accepted tradeoffs*) so future reviewers don't re-discover and re-file the finding. The code comment — not a ledger file — is the durable decline record, because it sits exactly where the next reviewer will look. Promotions, deferrals to a release folder and pull-forwards out of one are executed the same way: only when the human asks.

The gardener never promotes to `plan/` on its own, never moves a ticket between releases, and never declines without human direction — consolidation, ranking and proposals are autonomous; promotion, deferral and decline are the human's calls, made cheaper.

### Mechanics

`scripts/garden.mjs` hands the whole backlog to one agent invocation: the prompt is `agent-rules/garden.md` + the shared cross-stage conventions from `tickets.md` (stage blocks stripped) + the project rules addenda + a header-level inventory of every backlog ticket, sub-folders included, grouped by release (description, which triage headers and anchors are missing, and each top-level ticket's age) + the board check's errors and warnings, which garden reports rather than stopping on + the human feedback, when given (`--feedback <file>` and/or bare CLI arguments). Ages come from one `git log --name-status` walk over `tickets/backlog` (`lib/backlog-age.mjs`), not one query per ticket. The agent reads full ticket bodies only for what it acts on. Model selection runs through the shared resolver with stage `garden` and difficulty `hard` by default — gardening is judgment-dense. Output lands in `tickets/.garden-report.md` (tracked, overwritten per pass); the runner commits as `tess: garden backlog (<n> removed, <m> added, <k> updated)`, counting removals/additions by slug across `backlog/` and its sub-folders so neither re-sequencing nor a move between folders is miscounted as churn. The mass-deletion commit guard applies as everywhere else.
