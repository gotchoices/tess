...
│   └── LOCAL_RULES.md       # Optional, project-owned overrides
...
- `tickets/LOCAL_RULES.md` — optional project overrides (cwd-relative, appended if exists)
...
### Should tess detect and warn if it's out of date?

The runner could check `git -C tess log --oneline -1` against a known "minimum version" or just print the current tess commit hash at startup for awareness. Low priority but useful for multi-user setups where one developer updates and the other doesn't.

### Should init create a project-root `CLAUDE.md` as well as `AGENTS.md`?

Per Q9, the answer is yes — both convention files at the root, both pointing to tess. The root files are the project's own (user can add project-wide rules), with tess adding its section to each.
