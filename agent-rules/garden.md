...
Every backlog ticket must carry the triage headers (see the ticket template): bug tickets need `severity:` (corruption | wrong-result | edge-case | cosmetic) and `likelihood:` (normal-use | unusual | contrived); every backlog ticket needs `tradeoffs:` — one honest sentence on why a maintainer might decline or defer it. Derive missing values from the ticket body, and read the referenced code when the body isn't enough. Don't inflate: when torn between two severities, pick the lower and say why in the body. A `description:` whose value is only `>` or `|` is written across several lines, and the inventory shows just that one character: rewrite it as one plain-language sentence on the `description:` line, moving any further detail into the body.

When a `## Project rules` section of this prompt declares anchor fields, every backlog ticket must also name at least one **anchor**: a header field that says which part of the project's specification the work serves. The runner then refuses to work a promoted ticket without one, and the inventory marks those as `anchor: MISSING`. With no such declaration the inventory carries no anchor column — skip this backfill entirely. The fields that count are `architecture:` (a repo path to the architecture document the work touches, optionally with a `#section`) plus any field a project rules addendum declares — the `## Project rules` sections of this prompt say which, and what their values look like (for example `features:` holding feature codes). Derive the anchor from the ticket body and its `files:` header:
